...
class AppDatabase {
  ...
  Future<List<Subject>> getSubjects() async => (await (await database).query('subjects', orderBy: 'name COLLATE NOCASE')).map(Subject.fromMap).toList();
  ...
  Future<void> deleteSubject(int id) async => (await database).delete('subjects', where: 'id = ?', whereArgs: [id]);

  Future<List<AcademicTask>> getTasks() async => (await (await database).query('tasks', orderBy: 'due_date ASC')).map(AcademicTask.fromMap).toList();
  ...
  Future<List<Grade>> getGrades() async => (await (await database).query('grades', orderBy: 'date DESC')).map(Grade.fromMap).toList();
  ...
  Future<List<ScheduleEntry>> getSchedules() async => (await (await database).query('schedules', orderBy: 'day ASC, start_time ASC')).map(ScheduleEntry.fromMap).toList();
  ...
  Future<List<MaterialResource>> getMaterials() async => (await (await database).query('materials', orderBy: 'created_at DESC')).map(MaterialResource.fromMap).toList();
