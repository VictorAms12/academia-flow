...
class _AttendanceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final riskColor = switch (risk) {
      ...
      'RISCO' => Colors.orange,
      'SEM DADOS' => Theme.of(context).colorScheme.outline,
      _ => AppColors.danger,
    };
    return SoftCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(
          subject.hasAttendanceHistory
              ? 'meta ${target.toStringAsFixed(0)}% • ${state.completedClassCount(subject)} aulas registradas'
              : 'Nenhuma aula registrada ainda',
          style: Theme.of(context).textTheme.bodySmall,
        ),
        const SizedBox(height: 8),
        LinearProgressIndicator(
          value: subject.hasAttendanceHistory ? (attendance / 100).clamp(0, 1) : 0,
          minHeight: 7,
      ...
      LayoutBuilder(builder: (context, constraints) {
        final cards = [
          ...
          _InsightCard(
            ...
            value: state.onTimeTaskRate == null ? '—' : '${state.onTimeTaskRate!.toStringAsFixed(0)}%',
