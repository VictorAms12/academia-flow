...
class _LibraryScreenState extends State<LibraryScreen> with SingleTickerProviderStateMixin {
  ...
  void _tabChanged() {
    if (tabs.indexIsChanging || !mounted) return;
    setState(() {
      if (tabs.index != 0) {
        tagFilter = null;
        pinnedOnly = false;
      }
      if (tabs.index != 1) materialKindFilter = null;
      if (tabs.index != 2) attachmentKindFilter = null;
    });
  }
  ...
  bool get _hasActiveFilters {
    if (query.isNotEmpty || subjectFilter != null) return true;
    if (tabs.index == 0) return tagFilter != null || pinnedOnly;
    if (tabs.index == 1) return materialKindFilter != null;
    return attachmentKindFilter != null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final allTags = <String>{
      for (final note in state.notes) ...note.tags.split(',').map((tag) => tag.trim()).where((tag) => tag.isNotEmpty),
    }.toList()
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: switch (tabs.index) {
                  0 => 'Buscar em anotações, tags e conteúdo...',
                  1 => 'Buscar em materiais, links e descrições...',
                  _ => 'Buscar fotos e arquivos...',
                },
          ...
          Expanded(
            child: TabBarView(
              controller: tabs,
              children: [_notes(state), _materials(state), _attachments(state)],
            ),
          ),
        ],
      ),
  ...
  Widget _commonFilters(AppState state) => SizedBox(
        height: 50,
        child: ListView(
  ...
  Widget _noteFilters(List<String> tags) => SizedBox(
        height: 48,
        child: ListView(
          scrollDirection: Axis.horizontal,
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 3),
          children: [
            FilterChip(selected: pinnedOnly, avatar: const Icon(Icons.push_pin_outlined, size: 16), label: const Text('Fixadas'), onSelected: (value) => setState(() => pinnedOnly = value)),
            for (final tag in tags) ...[
              const SizedBox(width: 6),
              FilterChip(selected: tagFilter == tag, label: Text(tag), onSelected: (selected) => setState(() => tagFilter = selected ? tag : null)),
            ],
          ],
        ),
      );

  Widget _materialFilters() => _choiceStrip<MaterialKind>(
        selected: materialKindFilter,
        values: MaterialKind.values,
        label: materialKindLabel,
        onChanged: (value) => setState(() => materialKindFilter = value),
      );

  Widget _attachmentFilters() => _choiceStrip<AttachmentKind>(
        selected: attachmentKindFilter,
        values: AttachmentKind.values,
        label: attachmentKindLabel,
        onChanged: (value) => setState(() => attachmentKindFilter = value),
      );

  Widget _choiceStrip<T>({required T? selected, required List<T> values, required String Function(T) label, required ValueChanged<T?> onChanged}) => SizedBox(
        height: 48,
        child: ListView(
          scrollDirection: Axis.horizontal,
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 3),
          children: [
            ChoiceChip(selected: selected == null, label: const Text('Todos'), onSelected: (_) => onChanged(null)),
            for (final value in values) ...[
              const SizedBox(width: 6),
              ChoiceChip(selected: selected == value, label: Text(label(value)), onSelected: (_) => onChanged(value)),
            ],
          ],
        ),
      );
  ...
  Widget _notes(AppState state) {
    final items = state.notes.where((note) {
      ...
      return query.trim().isEmpty || matchesSearch(query, [note.title, note.content, note.tags, note.link, state.subjectName(note.subjectId)]);
    }).toList();
    items.sort((a, b) {
      if (a.pinned != b.pinned) return a.pinned ? -1 : 1;
      return _compare(a.title, b.title, state.subjectName(a.subjectId), state.subjectName(b.subjectId), a.createdAt, b.createdAt);
    });
    if (items.isEmpty) return _empty('Nenhuma anotação encontrada', Icons.note_alt_outlined, state);
          ...
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  if (note.pinned) ...[const Icon(Icons.push_pin_rounded, size: 17, color: AppColors.gold), const SizedBox(width: 6)],
                  ...
                  PopupMenuButton<String>(
                    onSelected: (value) async {
                      ...
                      if (value == 'pin') await _togglePin(state, note);
                ...
                if (noteTags.isNotEmpty) ...[
                  ...
                  Wrap(spacing: 6, runSpacing: 5, children: [
                    for (final tag in noteTags) ActionChip(visualDensity: VisualDensity.compact, label: Text(tag), onPressed: () => setState(() => tagFilter = tag)),
                  ]),
                ],
              ]),
            ),
  ...
  Future<void> _togglePin(AppState state, AcademicNote note) => state.saveNote(AcademicNote(
        id: note.id,
  ...
  Widget _materials(AppState state) {
    final items = state.materials.where((material) {
      ...
      return query.trim().isEmpty || matchesSearch(query, [material.title, material.description, material.url, materialKindLabel(material.kind), state.subjectName(material.subjectId)]);
    }).toList()
      ..sort((a, b) => _compare(a.title, b.title, state.subjectName(a.subjectId), state.subjectName(b.subjectId), a.createdAt, b.createdAt));
    if (items.isEmpty) return _empty('Nenhum material encontrado', Icons.folder_open_outlined, state);
      ...
      itemBuilder: (_, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            leading: CircleAvatar(backgroundColor: AppColors.gold.withValues(alpha: .10), child: Icon(_materialIcon(material.kind), color: AppColors.gold)),
            title: Text(material.title, style: const TextStyle(fontWeight: FontWeight.w900)),
            subtitle: Text('${state.subjectName(material.subjectId)} • ${materialKindLabel(material.kind)}${material.description.isEmpty ? '' : '\n${material.description}'}', maxLines: 3, overflow: TextOverflow.ellipsis),
            ...
            trailing: PopupMenuButton<String>(
              onSelected: (value) async {
                ...
                if (value == 'open' && _canOpen(material.url)) await launchUrl(Uri.parse(material.url), mode: LaunchMode.externalApplication);
                if (value == 'attachments' && material.id != null) {
                  await showAttachmentManager(context, target: AttachmentTarget(type: AttachmentTargetType.material, id: material.id!, subjectId: material.subjectId), title: material.title);
                  await _reloadAttachments();
                }
              ...
              itemBuilder: (_) => [
                const PopupMenuItem(value: 'edit', child: Text('Editar')),
                if (_canOpen(material.url)) const PopupMenuItem(value: 'open', child: Text('Abrir link')),
  ...
  Widget _attachments(AppState state) => FutureBuilder<List<AcademicAttachment>>(
        future: attachmentFuture,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
          if (snapshot.hasError) return _empty('Não foi possível carregar os anexos', Icons.error_outline_rounded, state, error: true);
          final items = (snapshot.data ?? const <AcademicAttachment>[]).where((item) {
            ...
            return query.trim().isEmpty || matchesSearch(query, [item.title, item.fileName, attachmentKindLabel(item.kind), state.subjectName(item.subjectId)]);
          }).toList()
            ..sort((a, b) => _compare(a.title, b.title, state.subjectName(a.subjectId), state.subjectName(b.subjectId), a.createdAt, b.createdAt));
          if (items.isEmpty) return _empty('Nenhum anexo encontrado', Icons.attach_file_rounded, state);
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(backgroundColor: AppColors.gold.withValues(alpha: .10), child: Icon(_attachmentIcon(item.kind), color: AppColors.gold)),
                    title: Text(item.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900)),
                    subtitle: Text('${state.subjectName(item.subjectId)} • ${attachmentTargetLabel(item.targetType)} • ${attachmentKindLabel(item.kind)}\n${item.fileName}', maxLines: 2, overflow: TextOverflow.ellipsis),
  ...
  Widget _empty(String title, IconData icon, AppState state, {bool error = false}) => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(error ? 'Tente atualizar novamente.' : _hasActiveFilters ? 'Ajuste a busca ou remova algum filtro.' : tabs.index == 2 ? 'Os anexos adicionados em aulas e conteúdos aparecerão aqui.' : 'Adicione seu primeiro conteúdo à Biblioteca.', textAlign: TextAlign.center),
            const SizedBox(height: 14),
            if (error)
              OutlinedButton.icon(onPressed: _reloadAttachments, icon: const Icon(Icons.refresh_rounded), label: const Text('Tentar novamente'))
            else if (!_hasActiveFilters && tabs.index < 2)
              FilledButton.icon(onPressed: () => tabs.index == 0 ? showNoteEditor(context, state) : showMaterialEditor(context, state), icon: const Icon(Icons.add_rounded), label: Text(tabs.index == 0 ? 'Nova anotação' : 'Novo material')),
          ]),
        ),
      );
