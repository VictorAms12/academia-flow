  ...
  Widget build(BuildContext context) {
    final columns = [
      _KanbanColumn(
        ...
        state: state,
        entranceOffset: const Offset(-.025, 0),
      ),
      _KanbanColumn(
        title: 'Em Andamento',
        ...
        state: state,
        entranceOffset: const Offset(0, .025),
      ),
      _KanbanColumn(
        title: 'Concluído',
              ...
              for (var i = 0; i < columns.length; i++) ...[
                Expanded(
                  child: MotionEntrance(
                    ...
                    offset: columns[i].entranceOffset,
                    child: columns[i],
                  ),
                ),
            ...
            Row(
              children: [
                ...
                AnimatedSwitcher(
                  ...
                  child: GoldBadge('${tasks.length}', key: ValueKey(tasks.length)),
                ),
              ],
            ),
...
class _TaskCardState extends State<_TaskCard> {
    ...
    return AnimatedScale(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: glow
              ? [BoxShadow(blurRadius: 28, spreadRadius: 2, color: AppColors.gold.withValues(alpha: .30))]
              : null,
        ),
        child: SoftCard(
            ...
            children: [
                ...
                children: [
                  ...
                  AnimatedSwitcher(
                    ...
                    child: task.status == TaskStatus.done
                        ? const Icon(Icons.check_circle_rounded, key: ValueKey('done'), color: AppColors.success, size: 19)
                  ...
                  PopupMenuButton<String>(
                    onSelected: (value) async {
                      ...
                      if (value == 'delete' && await confirmDelete(context, task.title)) {
              ...
              AnimatedDefaultTextStyle(
                ...
                style: TextStyle(
                  ...
                  fontSize: 15,
                  decoration: task.status == TaskStatus.done ? TextDecoration.lineThrough : TextDecoration.none,
                  decorationColor: Theme.of(context).textTheme.bodyLarge?.color?.withValues(alpha: .55),
                ),
                    ...
                    child: Row(
                      children: [
                        ...
                        AnimatedSwitcher(
                          duration: MotionSpec.fast,
                          child: Text(
                            '${task.completedSteps.length}/${task.checklist.length}',
                            key: ValueKey(task.completedSteps.length),
                            style: Theme.of(context).textTheme.labelSmall?.copyWith(fontWeight: FontWeight.w800),
                          ),
                        ),
                        const SizedBox(width: 3),
                        AnimatedRotation(
                          turns: checklistOpen ? .5 : 0,
                          duration: MotionSpec.fast,
                          curve: MotionSpec.curve,
              ...
              Row(
                children: [
                  Icon(
                    ...
                    size: 15,
                    color: days <= 1 && task.status != TaskStatus.done ? AppColors.danger : null,
                  ),
  ...
  Future<void> _toggleStep(AcademicTask task, int step) async {
    ...
    if (finishesChecklist && mounted) setState(() => glow = true);
    await widget.state.toggleTaskStep(task, step);
    if (finishesChecklist) {
      await Future<void>.delayed(const Duration(milliseconds: 360));
      if (mounted) {
        setState(() => glow = false);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Checklist completo ✨')),
        );
      }
    }
  }
...
class _ChecklistRow extends StatelessWidget {
  const _ChecklistRow({super.key, required this.label, required this.checked, required this.onChanged});
  final String label;
  final bool checked;
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Checkbox(value: checked, onChanged: (_) => onChanged(), visualDensity: VisualDensity.compact),
            ...
            AnimatedSwitcher(
              ...
              transitionBuilder: (child, animation) => ScaleTransition(scale: animation, child: child),
              child: checked
                  ? const Icon(Icons.check_rounded, key: ValueKey(true), size: 16, color: AppColors.success)
                  : const SizedBox(width: 16, key: ValueKey(false)),
            ),
          ],
        ),
      ),
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text('${months[now.month]} ${now.year}', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
              ),
            ],
          ),
                      ...
                      if (count > 0)
                        Container(
                          ...
                          child: Text(
                            ...
                            style: const TextStyle(fontSize: 7, color: AppColors.gold, fontWeight: FontWeight.w900),
                          ),
                        ),
                    ],
                  ),
